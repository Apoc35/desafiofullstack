...
var batchRequests = (...args) => __async(void 0, null, function* () {
...
var runRequest = (input) => __async(void 0, null, function* () {
  const config = __spreadProps(__spreadValues({}, input), {
    ...
    fetchOptions: __spreadProps(__spreadValues({}, input.fetchOptions), {
      errorPolicy: input.fetchOptions.errorPolicy ?? `none`
    })
  });
  const fetcher = createFetcher(config.method);
  const fetchResponse = yield fetcher(config);
  if (!fetchResponse.ok) {
  ...
  const result = yield parseResultFromResponse(fetchResponse, input.fetchOptions.jsonSerializer ?? defaultJsonSerializer);
  ...
  if (isRequestResultHaveErrors(result) && config.fetchOptions.errorPolicy === `none`) {
    const clientResponse = result._tag === `Batch` ? __spreadValues(__spreadValues({}, result.executionResults), clientResponseBase) : __spreadValues(__spreadValues({}, result.executionResult), clientResponseBase);
  ...
  switch (result._tag) {
    case `Single`:
      return __spreadValues(__spreadValues({}, clientResponseBase), executionResultClientResponseFields(config)(result.executionResult));
    case `Batch`:
      return __spreadProps(__spreadValues({}, clientResponseBase), {
...
var parseResultFromResponse = (response, jsonSerializer) => __async(void 0, null, function* () {
...
var createFetcher = (method) => (params) => __async(void 0, null, function* () {
  ...
  const init = __spreadValues({ method, headers, body }, params.fetchOptions);
  let url = new URL(params.url);
  let initResolved = init;
  if (params.middleware) {
    const result = yield Promise.resolve(params.middleware(__spreadProps(__spreadValues({}, init), {
      url: params.url,
      operationName: params.request._tag === `Single` ? params.request.document.operationName : void 0,
      variables: params.request.variables
    })));
    const _a = result, { url: urlNew } = _a, initNew = __objRest(_a, ["url"]);
    url = new URL(urlNew);
  ...
  return yield $fetch(url, initResolved);
});
...
var GraphQLClient = class {
   ...
   */
  rawRequest = (...args) => __async(this, null, function* () {
    const [queryOrOptions, variables, requestHeaders] = args;
    const rawRequestOptions = parseRawRequestArgs(queryOrOptions, variables, requestHeaders);
    const _a = this.requestConfig, { headers, fetch: fetch2 = globalThis.fetch, method = `POST`, requestMiddleware, responseMiddleware, excludeOperationName } = _a, fetchOptions = __objRest(_a, ["headers", "fetch", "method", "requestMiddleware", "responseMiddleware", "excludeOperationName"]);
    const { url } = this;
    ...
    const response = yield runRequest({
      ...
      headers: __spreadValues(__spreadValues({}, HeadersInitToPlainObject(callOrIdentity(headers))), HeadersInitToPlainObject(rawRequestOptions.requestHeaders)),
      fetch: fetch2,
      method,
      fetchOptions,
    ...
    if (responseMiddleware) {
      yield responseMiddleware(response, {
  ...
  request(documentOrOptions, ...variablesAndRequestHeaders) {
    return __async(this, null, function* () {
      ...
      const response = yield runRequest({
        ...
        headers: __spreadValues(__spreadValues({}, HeadersInitToPlainObject(callOrIdentity(headers))), HeadersInitToPlainObject(requestOptions.requestHeaders)),
        fetch: fetch2,
        method,
        fetchOptions,
        middleware: requestMiddleware
  ...
  batchRequests(documentsOrOptions, requestHeaders) {
    return __async(this, null, function* () {
      ...
      const _a = this.requestConfig, { headers, excludeOperationName } = _a, fetchOptions = __objRest(_a, ["headers", "excludeOperationName"]);
      ...
      const response = yield runRequest({
        ...
        headers: __spreadValues(__spreadValues({}, HeadersInitToPlainObject(callOrIdentity(headers))), HeadersInitToPlainObject(batchRequestOptions.requestHeaders)),
      ...
      if (this.requestConfig.responseMiddleware) {
        yield this.requestConfig.responseMiddleware(response, {
          operationName: void 0,
          variables,
          url: this.url
        });
...
function request(urlOrOptions, document, ...variablesAndRequestHeaders) {
  return __async(this, null, function* () {
    const requestOptions = parseRequestExtendedArgs(urlOrOptions, document, ...variablesAndRequestHeaders);
    const client = new GraphQLClient(requestOptions.url);
    return client.request(__spreadValues({}, requestOptions));
  });
}
